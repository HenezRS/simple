...
public final class ButtonFactory {
    ...
    public static ButtonGroup createCharacterSelectButtons() {
        ...
        Arrays.stream(ClassName.values()).forEach(job -> {
            buttonGroup.addButton(new ImageButton(job.getName(), parentX + (x.getAndIncrement() * 22), parentY + (y.get() * 22), job.getImgSet().getPortrait().getTex(), ImageButtonDefinitions.box20));
            if (x.get() == 5) {
                y.getAndIncrement();
                x.set(0);
            }
